...
export default function ServicesList() {
    ...
    const handleYesClick = () => {
        setLoading(true);
        axios
            .delete(`/deleteService?i=${toDelId}`)
            .then(op => {
                // console.log("I am output::", op);
                setLoading(false);
                if (!_.isEmpty(op) && !_.isEmpty(op.data) && op.data.message === 'SERVICE_DELTED') {
                    loadList();
    ...
    return (<>
                    ...
                    <div className="col-12">
                        <div className="card mb-4">
                            <div className="card-header pb-0 d-flex justify-content-between align-items-center">
                                <h6 className="font-weight-bolder">Services List</h6>
                                        ...
                                        <tbody>
                                            {
                                                !_.isEmpty(servicesList) ? servicesList.map(
                                                    (el, index) =>
                                                        <tr key={`service-${el.serviceId}-${index}`}>
                                                            <td><span className="px-3 text-xs">{el.serviceId}</span></td>
                                                            ...
                                                            <td className="">
                                                                ...
                                                                <a href="/" onClick={(e) => handleDelClick(e, el.serviceId)} className="text-secondary font-weight-bold text-xs text-danger" data-toggle="tooltip" data-original-title="Edit user">
        ...
        >
            <Popover>
                ...
                <Popover.Body >
                    <p className="text-xs mb-2">Confirm delete service</p>
